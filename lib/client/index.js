    ...
    class qxClient {
        ...
        cors(options) {
            ...
            return function*(next) {
                ...
                this.set('Access-Control-Allow-Methods', options.allowMethods || self.defaultCorsOptions.allowMethods);
                    ...
                    if (opts.multipart && self.is('multipart')) {
                        ...
                        body = yield new Promise(function(resolve, reject) {
                            req.on('readable', () => {
                                ...
                                if (self.is('text')) {
                                    len > (opts.textLimit || opts.limit) ? resolve('') : resolve(decodeURIComponent(chunk.toString('utf-8', 0)));
                                } else if (self.is('json')) {
                                    len > (opts.jsonLimit || opts.limit) ? resolve('') : resolve(JSON.parse(decodeURIComponent(chunk.toString('utf-8', 0))));
                                } else if (self.is('urlencoded')) {
                                    var str = '';
                                    len > (opts.formLimit || opts.limit) ? resolve('') : str = decodeURIComponent(chunk.toString('utf-8', 0));
                                    str.indexOf('=') > -1 ?
