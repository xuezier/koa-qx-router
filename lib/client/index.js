(function() {
  ...
  class qxClient {
    ...
    auth(auth) {
      ...
      return function(req, res, next) {
        ...
        if(`${_auth.name}:${_auth.pass}` === authorization) {
          ...
          res.status(401).end('can no auth');
          // this.throw(401);
        }
      };
    }
    registerServer(app) {
      var registerBefore = (typeof(app) === 'function');
      if(!registerBefore) {
    ...
    bodyParser(opts) {
      opts = opts || {};
      opts.patchNode = 'patchNode' in opts ? opts.patchNode : true;
      ...
      return function(req, res, next) {
        ...
        co(function*() {
          if(!opts.strict || ['GET', 'HEAD'].indexOf(method) === -1) {
            ...
            opts.patchNode ? req.body = body : false;
          }
          next();
        }).catch(e => {
          next(e);
